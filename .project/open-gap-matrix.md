...
| GAP-003 | High | RC notes describe an earlier pass | Current dirty source has no fresh complete `pnpm validate` evidence | 1, 13 | open |
...
| GAP-010 | High | Current secret assignment pattern matches fixtures/locales | Security gate is noisy and would fail despite no provider-token finding | 9 | open |
